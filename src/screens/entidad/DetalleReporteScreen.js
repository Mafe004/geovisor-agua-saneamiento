...
export default function DetalleReporteScreen({ route, navigation }) {
  ...
  const handleUpdateEstado = async () => {
    ...
    try {
      ...
      setComentario('');
      Alert.alert('✅ Estado actualizado', `El reporte ahora está en: ${nuevoEstado.replace('_', ' ')}`);
    } catch (e) {
  ...
  if (!reporte) {
    return (
      <View style={styles.center}>
        <Text>No se encontró información del reporte.</Text>
      </View>
    );
  }

  const hasCoords = reporte.latitud && reporte.longitud &&
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Section title="ℹ️ Detalles">
          ...
          <DetailRow label="Reportado por" value={reporte.usuario_nombre ? `${reporte.usuario_nombre} ${reporte.usuario_apellido || ''}` : '—'} />
          ...
          <Section title="🔄 Cambiar Estado">
            <View style={styles.estadoGrid}>
              {ESTADOS_DISPONIBLES.map(e => (
                <TouchableOpacity
                  key={e.key}
                  style={[styles.estadoChip, nuevoEstado === e.key && styles.estadoChipActive]}
                  onPress={() => setNuevoEstado(e.key)}
                >
                  <Text style={[styles.estadoChipText, nuevoEstado === e.key && styles.estadoChipTextActive]}>
                    {e.label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <TouchableOpacity
              ...
              disabled={updating}
            >
              <LinearGradient colors={['#1565C0', '#00ACC1']} style={styles.updateBtnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                {updating
